...
const plugin = {
    processEvent: async (event, { config }) => {
        ...
        if (parsedLibs && event.properties?.$lib && parsedLibs.includes(event.properties?.$lib)) {
            console.info(`Discarding GeoIP properties from ${event.uuid || event.event} as event comes from ignored $lib: ${event.properties?.$lib}.`);
            if (event.$set) {
        ...
        if (config.discardIp === 'true') {
            if (Array.isArray(event.properties?.$plugins_succeeded) &&
                event.properties?.$plugins_succeeded.find((val) => val.toString().match(GEO_IP_PLUGIN))) {
