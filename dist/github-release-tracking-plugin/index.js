...
async function runEveryMinute({ config, global, cache }) {
    ...
    const ghReleasesRes = await fetch(`https://api.github.com/repos/${config.ghOwner}/${config.ghRepo}/releases`, global.ghOptions);
    ...
    for (let release of newReleases) {
        ...
        if (createAnnotationRes.status === 201) {
            ...
            console.error('failed to create annotation', release.name, ' with status ', createAnnotationRes.status, ' and error ', errorMessage);
        }
    }
