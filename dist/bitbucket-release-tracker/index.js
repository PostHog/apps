async function setupPlugin({ config, global }) {
    ...
    if (config.bitbucketUsername && config.bitbucketToken) {
        bitbucketBase64AuthToken = Buffer.from(`${config.bitbucketUsername}:${config.bitbucketToken}`).toString('base64');
    }
    else if (config.bitbucketUsername || config.bitbucketToken) {
        throw new Error('Please supply both a Bitbucket username and a personal token to use this plugin with private repos');
    }
...
async function runEveryMinute({ config, global, cache }) {
    ...
    if (lastRun &&
        new Date().getTime() - Number(lastRun) < 3600000) {
        return;
    }
    ...
    const bitbucketTagsResponse = await fetchWithRetry(`${global.bitbucketApiBaseUrl}/refs/tags`, global.bitbucketAuthHeader);
