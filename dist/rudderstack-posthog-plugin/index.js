...
async function setupPlugin({ config, global, jobs }) {
    ...
    global.buffer = createBuffer({
        ...
        timeoutSeconds: 60,
        onFlush: async (batch) => {
            await sendToRudder({ batch, retriesPerformedSoFar: 0, batchId: Math.floor(Math.random() * 1000000) }, { global, jobs });
        },
    });
}
async function onEvent(event, { global }) {
    let rudderPayload = {};
...
async function sendToRudder(batch, { global, jobs }) {
    try {
        ...
        const nextRetryMs = 2 ** (batch.retriesPerformedSoFar || 0) * 3000;
...
export { jobs, onEvent, setupPlugin };
