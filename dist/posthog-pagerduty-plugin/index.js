...
async function isTrendErroring(meta) {
    ...
    const result = (_a = insight.result) === null || _a === void 0 ? void 0 : _a[0];
    if (!result) {
    ...
    return latestDataPoints.every((value) => dataPointInError(value, parseFloat(meta.config.threshold), meta.config.operator));
}
...
async function getTrend(meta) {
    ...
    const apiUrl = new URL(`/api/projects/${meta.config.posthogProjectId}/insights?short_id=${insightId}`, meta.config.posthogTrendUrl);
