async function setupPlugin({ config }) {
    const validationRes = await fetch(`https://cdn.syndication.twimg.com/widgets/followbutton/info.json?screen_names=${config.twitterHandle}`);
...
async function runEveryDay({ config }) {
    config.twitterHandle =
        config.twitterHandle[0] === '@'
            ...
            : config.twitterHandle;
    const followersResponse = await fetchWithRetry(`https://cdn.syndication.twimg.com/widgets/followbutton/info.json?screen_names=${config.twitterHandle}`);
...
var twitterFollowersPlugin = {
    setupPlugin,
    runEveryDay
};

export { twitterFollowersPlugin as default };
