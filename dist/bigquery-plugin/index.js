...
const setupPlugin = async (meta) => {
    ...
    try {
        ...
        if (error instanceof FetchError) {
            throw new RetryError(`Operational fetch error encountered: ${error.message}`);
        }
        else {
            throw error;
        }
...
async function updateBigQueryTableSchema(meta, metadata) {
    ...
    try {
        ...
        const fieldsToAdd = BIG_QUERY_TABLE_FIELDS.filter(({ name }) => !existingFields.find((f) => f.name === name));
        if (fieldsToAdd.length > 0) {
            console.info(`Incomplete schema on BigQuery table! Adding the following fields to reach parity: ${JSON.stringify(fieldsToAdd)}`);
            let result;
            try {
...
async function createBigQueryTable(meta) {
    const { global, config } = meta;
    ...
    try {
        ...
        if (!error.message.includes('Already Exists')) {
            throw new RetryError(`Another thread aleady created the table, retrying setup (${error.message})`);
        }
...
const exportEvents = async (events, { global, config }) => {
    ...
    try {
        ...
        throw new RetryError(`Error inserting into BigQuery! ${error.message}`);
    }
};

export { BIG_QUERY_TABLE_FIELDS, exportEvents, setupPlugin };
