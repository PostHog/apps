...
const defaultTransformation = (str, matchPattern, sep) => {
    const parsedStr = str.replace(matchPattern, (substr) => sep + (substr.length === 1 ? substr.toLowerCase() : substr[1].toLowerCase()));
    if (parsedStr[0] === sep) {
        return parsedStr.slice(1);
    }
    return parsedStr;
};
...
export { taxonomyPlugin as default };
