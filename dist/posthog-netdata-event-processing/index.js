...
function isDemo(url) {
    if ((url.includes('://london.my-netdata.io'))
        ||
        ...
        event.properties['$elements'].slice().reverse().forEach((element) => {
            ...
            if ('$el_text' in element && element['$el_text'] !== null && element['$el_text'] !== '') {
                ...
                if (element['$el_text'].includes('/20') && isStringDDMMYYYYHHMM(element['$el_text'])) {
                    ...
                    dtStrClean = dtStr.substring(6, 10).concat('-', dtStr.substring(3, 5), '-', dtStr.substring(0, 2), ' ', dtStr.substring(11, 16));
...
function getInteractionTypeAgent(event) {
    ...
    else if (event.properties.hasOwnProperty('el_data_netdata') &&
    ...
    else if (event.properties.hasOwnProperty('el_class_collapsablesection') ||
        event.properties['el_title'] === 'hamburger') {
        return 'hamburger';
    }
    else if (event.properties.hasOwnProperty('el_data_target_updatemodal') ||
        event.properties.hasOwnProperty('el_id_updatemodal')) {
        return 'update';
    }
    else if (['Need Help?', 'question'].includes(event.properties['el_title']) ||
        event.properties['el_data_testid'] === 'documentation-help-close' ||
        event.properties.hasOwnProperty('el_class_documentation_container')) {
        return 'help';
    }
    else if (event.properties['el_data_target'] === '#loadSnapshotModal' ||
    ...
    else if (event.properties['el_data_target'] === '#saveSnapshotModal' ||
    ...
    else if (event.properties['el_data_target'] === '#printPreflightModal' ||
        event.properties['el_onclick'] === 'return printPreflight(),!1') {
        return 'print';
    }
    else if (event.properties['el_data_target'] === '#alarmsModal' ||
    ...
    else if (event.properties['el_data_target'] === '#optionsModal' ||
...
function getInteractionDetailAgent(event) {
    if (['menu', 'submenu'].includes(event.properties['interaction_type'])) {
        ...
        else if (event.properties.hasOwnProperty('el_data_testid') &&
            event.properties['el_data_testid'].startsWith('date-picker')) {
        ...
        if (event.properties.hasOwnProperty('el_href') &&
        ...
        else if (event.properties.hasOwnProperty('el_class_warning') &&
            event.properties.hasOwnProperty('el_text')) {
            if (event.properties['el_text'].includes(':') ||
        ...
        else if (event.properties.hasOwnProperty('el_class_success') &&
            event.properties.hasOwnProperty('el_text')) {
            if (event.properties['el_text'].includes(':') ||
                event.properties['el_text'].includes('%')) {
                return 'norm';
        ...
        else if (event.properties.hasOwnProperty('el_class_danger') &&
            event.properties.hasOwnProperty('el_text')) {
            if (event.properties['el_text'].includes(':') ||
                event.properties['el_text'].includes('%')) {
                return 'crit';
        ...
        else if (event.properties.hasOwnProperty('el_class_info') &&
            event.properties.hasOwnProperty('el_text')) {
            if (event.properties['el_text'].includes(':') ||
                event.properties['el_text'].includes('%')) {
                return 'undef';
        ...
        else if (event.properties['el_title'] === 'Refresh' &&
            event.properties['el_id'] === 'alarms_log') {
            return 'refresh_log';
        }
        else {
...
var posthogNetdataEventProcessing = {
    setupPlugin,
    processEvent,
};

export { posthogNetdataEventProcessing as default };
