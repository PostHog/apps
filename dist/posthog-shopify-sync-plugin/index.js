async function setupPlugin({ config, global, storage }) {
    ...
    const authResponse = await fetchWithRetry(`https://${config.shopifyStore}.myshopify.com/admin/api/2022-01/orders.json?limit=1`, global.defaultHeaders);
...
async function fetchAllOrders(shopifyStore, defaultHeaders, orderApiUrl, cache, storage) {
    ...
    while (hasMoreOrders) {
        ...
        const newOrders = (orderJson === null || orderJson === void 0 ? void 0 : orderJson.orders) || [];
...
function getNextPageUrl(headers) {
    if (headers === null || headers === void 0 ? void 0 : headers.has('link')) {
        ...
        for (let i = 0; i < paginationInfo.length; i++) {
            ...
            if (paginatedLinkBody[1].trim() === 'rel="next"') {
                const orderApiUrl = paginatedLinkBody[0].substring(paginatedLinkBody[0].indexOf('<') + 1, paginatedLinkBody[0].lastIndexOf('>'));
                return orderApiUrl;
            }
        }
    }
...
export { posthogShopifySyncPlugin as default };
