...
export const setupPlugin: BigQueryPlugin['setupPlugin'] = async (meta) => {
    ...
    try {
        ...
        if (
            cachedMetadata?.datasetId === config.datasetId &&
            cachedMetadata?.tableId === config.tableId &&
            cachedMetadata?.existingFields === BIG_QUERY_TABLE_FIELDS.length
        ) {
            return
        }
    ...
    try {
        ...
        if (!(error as Error).message.includes('Already Exists')) {
            throw new RetryError(
                `Another thread aleady created the table, retrying setup (${(error as Error).message})`
            )
        }
